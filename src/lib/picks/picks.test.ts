...
describe("pick entry", () => {
  test("a member picks a winner one game at a time and can change it before the deadline", async () => {
    ...
    await savePick(db, grandma, michigan.id, michigan.homeTeamId, THURSDAY);
    await savePick(db, grandma, texas.id, texas.awayTeamId, THURSDAY);
    ...
    await savePick(db, grandma, texas.id, texas.homeTeamId, new Date("2026-09-10T21:00:00Z"));
    sheet = await pickSheet(db, grandma, week.id, THURSDAY);
  ...
  test("the server clock enforces the deadline on picks, the lock, and the guess", async () => {
    ...
    await savePick(db, grandma, michigan.id, michigan.awayTeamId, justBefore);
    ...
    for (const late of [deadline, new Date(deadline.getTime() + 3600_000)]) {
      await expect(savePick(db, grandma, texas.id, texas.homeTeamId, late)).rejects.toThrow(DeadlinePassed);
      await expect(savePick(db, grandma, michigan.id, michigan.homeTeamId, late)).rejects.toThrow(/deadline has passed/i);
  ...
  test("the Lock of the Week sits on one picked game at most and moves when re-chosen", async () => {
    ...
    await savePick(db, grandma, michigan.id, michigan.homeTeamId, THURSDAY);
    await savePick(db, grandma, texas.id, texas.homeTeamId, THURSDAY);
    await savePick(db, grandma, florida.id, florida.homeTeamId, THURSDAY);
    ...
    await savePick(db, jonah, michigan.id, michigan.awayTeamId, THURSDAY);
    await setLock(db, jonah, week.id, michigan.id, THURSDAY);
    expect((await pickSheet(db, grandma, week.id, THURSDAY)).lockGameId).toBe(texas.id);

    // A void game cannot take a Lock; clearing removes it.
  ...
  test("nobody reads another member's picks before the deadline, commissioners included", async () => {
    ...
    expect(revealed.map((m) => m.memberId).sort()).toEqual([jonah.id, grandma.id].sort());
  ...
  test("a pick must name a team in the game, on a live game, in a published week; a guess is a whole score", async () => {
    ...
    await expect(savePick(db, grandma, michigan.id, 999999, THURSDAY)).rejects.toThrow(/one of the two teams/i);
    await expect(savePick(db, grandma, 999999, michigan.homeTeamId, THURSDAY)).rejects.toThrow(/not on the slate/i);

    await voidGame(db, jonah, florida.id, "Hurricane");
    await expect(savePick(db, grandma, florida.id, florida.homeTeamId, THURSDAY)).rejects.toThrow(/void/i);
