 ...
 * session cookie, the feed, and the wall clock. It is the only piece of the
 * pick API that a test cannot run, which is the point — everything above it
 * takes this as an argument.
 */
export function pickRoute(): PickRoute {
  return { db: db(), currentMember, cfbd };
}
